#!/usr/bin/env python3
# 0__batch_run_complete_pipeline.py
# Complete pipeline runner for podcast transcription and knowledge extraction
# 
# Runs the complete pipeline:
# 0. Extract text from PDF/Office documents (0_batch_extract_txt_from_pdf_office_etc.py) [optional]
...
# 4. Extract entities with parallel processing (4b_batch_parallel_extract_entities_from_augmented_txts.py)
#
# Usage:
#   python 0__batch_run_complete_pipeline.py                     # Use default directories
#   python 0__batch_run_complete_pipeline.py --help             # Show help
...
def print_help():
    """Print usage help."""
    print("""
Complete Document and Podcast Processing Pipeline

Usage:
  python 0__batch_run_complete_pipeline.py [OPTIONS]

Options:
  --help, -h            Show this help message

Pipeline Steps:
  0. Extract text from PDFs/Office docs (0_batch_extract_txt_from_pdf_office_etc.py)
  1. Extract MP3 from videos           (1_batch_extract_mp3_from_video.py)
  2. Transcribe MP3 to text            (2_batch_whisper_split_then_transcribe_mp3.py)
  3. Augment with AI metadata          (3_batch_augment_transcripts.py)
  4. Extract entities parallel         (4b_batch_parallel_extract_entities_from_augmented_txts.py)

Default Directories:
  data/0_pdf_office_etc_source/ -> data/3_txt_transcribed/
...
Requirements:
  ...
  - Python packages: openai, tqdm, scikit-learn, numpy, tenacity, pymupdf, python-docx, python-pptx, markdown
...
def check_requirements():
    ...
    if missing_packages:
        ...
        errors.append("   Fix: pip install openai tqdm scikit-learn numpy tenacity pymupdf python-docx python-pptx markdown")
    ...
    if errors:
        ...
        print("   2. Install Python packages: pip install openai tqdm scikit-learn numpy tenacity pymupdf python-docx python-pptx markdown")  
...
def main():
    ...
    print("🎬 Starting Complete Document and Podcast Processing Pipeline")
    
    # Check requirements
    check_requirements()
    
    # Step 0: Extract text from PDF/Office documents
    success = run_script(
        "0_batch_extract_txt_from_pdf_office_etc.py",
        "Extract text from PDF and Office documents"
    )
    if not success:
        print("\n❌ Pipeline stopped due to error in step 0 (PDF extraction)")
        sys.exit(1)
    ...
    # Step 3: Augment with AI metadata
    success = run_script(
        "3_batch_augment_transcripts.py",
